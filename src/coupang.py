...
def initialize():
    """Initializes a session with Coupang, handling redirects and cookies."""
...
coupang_session = initialize()

if coupang_session:
    print("\nSession object obtained successfully.")
else:
    print("\nSession initialization failed.")

def search_item(session, query):
    """
    Searches for an item on Coupang using the provided session and query.

    Args:
        session: The requests.Session object obtained from initialize().
...
if coupang_session:
    ...
    search_query = "tomato 1kg"
    search_response = search_item(coupang_session, search_query)

    if search_response:
        print(f"Successfully received search results for '{search_query}'.")

        safe_query = "".join(c if c.isalnum() else "_" for c in search_query).strip("_")
        filename = f"./output/coupang_search_{safe_query}.html"
        try:
        ...
        print(f"Failed to get search results for '{search_query}'.")

else:
    print("\nFailed to initialize session. Cannot perform search.")